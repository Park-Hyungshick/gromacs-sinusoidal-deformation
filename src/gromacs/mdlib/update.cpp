                                 ...
                                 const bool                                       haveConstraints)
{
    ...
    // Calculate box deformation velocity
    // For linear deformation: use constant inputRecord.deform (efficient)
    // For sinusoidal deformation: calculate time-dependent velocity (required)
    matrix boxDeformationVelocity;
    if (deform_ != nullptr && inputRecord.deformType == DeformationType::Sinusoidal)
    {
        // Time-dependent velocity for sinusoidal deformation
        deform_->getVelocity(boxDeformationVelocity, step);
    }
    else
    {
        // For linear deformation, use constant rate from inputRecord.deform
        // For no deformation, this is already zero
