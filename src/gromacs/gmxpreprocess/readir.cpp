...
struct gmx_inputrec_strings
{
    char tcgrps[STRLEN], tau_t[STRLEN], ref_t[STRLEN], accelerationGroups[STRLEN],
            ...
            wall_density[STRLEN], deform[STRLEN], deform_sin_amplitude[STRLEN],
            deform_sin_period[STRLEN], QMMM[STRLEN], imd_grp[STRLEN];
            ...
            WarningHandler* wi)
{
    ...
    ir->deformType = getEnum<DeformationType>(&inp, "deform-type", wi);
    setStringEntry(&inp, "deform", inputrecStrings->deform, nullptr);
    setStringEntry(&inp, "deform-sin-amplitude", inputrecStrings->deform_sin_amplitude, nullptr);
    setStringEntry(&inp, "deform-sin-period", inputrecStrings->deform_sin_period, nullptr);
...
#undef CTYPE
    ...
    /* Parse sinusoidal deformation amplitude */
    clear_mat(ir->deform_sin_amplitude);
    for (i = 0; i < 6; i++)
    {
        dumdub[0][i] = 0;
    }
    if (strlen(inputrecStrings->deform_sin_amplitude) > 0)
    {
        int nsinamp = sscanf(inputrecStrings->deform_sin_amplitude,
                             "%lf %lf %lf %lf %lf %lf %lf",
                             &(dumdub[0][0]),
                             &(dumdub[0][1]),
                             &(dumdub[0][2]),
        ...
        {
            wi->addError(gmx::formatString(
                    "Cannot parse exactly 6 sinusoidal deformation amplitudes from string '%s'",
                    inputrecStrings->deform_sin_amplitude));
        }
        for (i = 0; i < 3; i++)
        {
            ir->deform_sin_amplitude[i][i] = dumdub[0][i];
        }
        ir->deform_sin_amplitude[YY][XX] = dumdub[0][3];
        ir->deform_sin_amplitude[ZZ][XX] = dumdub[0][4];
        ir->deform_sin_amplitude[ZZ][YY] = dumdub[0][5];
    }

    /* Parse sinusoidal deformation period */
    clear_mat(ir->deform_sin_period);
    for (i = 0; i < 6; i++)
    {
        dumdub[0][i] = 0;
    }
    if (strlen(inputrecStrings->deform_sin_period) > 0)
    {
        int nsinper = sscanf(inputrecStrings->deform_sin_period,
                             "%lf %lf %lf %lf %lf %lf %lf",
                             &(dumdub[0][0]),
                             &(dumdub[0][1]),
                             &(dumdub[0][2]),
        ...
        {
            wi->addError(gmx::formatString(
                    "Cannot parse exactly 6 sinusoidal deformation periods from string '%s'",
                    inputrecStrings->deform_sin_period));
        }
        for (i = 0; i < 3; i++)
        {
            ir->deform_sin_period[i][i] = dumdub[0][i];
        }
        ir->deform_sin_period[YY][XX] = dumdub[0][3];
        ir->deform_sin_period[ZZ][XX] = dumdub[0][4];
        ir->deform_sin_period[ZZ][YY] = dumdub[0][5];
    }
